...
Train_data_dnn = zeros(1, 64, 1, Training_set_ratio * total_numbers);
Train_label_dnn = zeros(1, 1, 64, Training_set_ratio * total_numbers);
Valid_data_dnn = zeros(1, 64, 1, total_numbers - Training_set_ratio * total_numbers);
Valid_label_dnn = zeros(1, 1, 64, total_numbers - Training_set_ratio * total_numbers);
...
for n  = 1 : N
    ...
    [rx_ofdm_data,rx_pofdm_data,synchron_local,synchron_proba,MSE_cfo] = rx_test1(synchron_local,synchron_proba,MSE_cfo,rx_signal,rx_perfect_signal,CarrierNUM,symbol_num,N_FFT,N_CP,Nsym_CP,cfo_offset,Fs,n);
    [PSDU_decode_out] = rx_test2(rx_ofdm_data,symbol_num,Pilot_method,VCNum_method,Np,loc_pilot,phase,N_FFT,ValidCarrierNum,PSDU_BPC,wlanSymMap,PadBits_Num,PSDU_PB);

    rx_nofdm_data = rx_ofdm_data - rx_pofdm_data;

    % 采集数据
    [Train_data_dnn,Train_label_dnn,Valid_data_dnn,Valid_label_dnn] = dl_dnn_data_collect(Train_data_dnn,Train_label_dnn,Valid_data_dnn,Valid_label_dnn,rx_ofdm_data,rx_nofdm_data,frame_cnt,total_numbers,Training_set_ratio);

    % 完美去噪后的信号解调
    % [PSDU_pdecode_out] = rx_test2(rx_pofdm_data,symbol_num,Pilot_method,VCNum_method,Np,loc_pilot,phase,N_FFT,ValidCarrierNum,PSDU_BPC,wlanSymMap,PadBits_Num,PSDU_PB);

    %% 深度学习时域降噪方法
    % 深度学习降噪后的信号解调
    rx_dnn_ofdm_data = rx_ofdm_data;
    for i = 1:symbol_num
        Dnn_Input = [real(rx_ofdm_data(i,:)) imag(rx_ofdm_data(i,:))];
        Dnn_Input_norm = 2*(Dnn_Input - min(Dnn_Input))./(max(Dnn_Input)-min(Dnn_Input))-1;
        Dnn_Input_norm_trs = Dnn_Input_norm.';
        Received_data_DNN = predict(Train_out,Dnn_Input_norm_trs);
        pre_Received_data_DNN = (Received_data_DNN + 1).*(max(Dnn_Input)-min(Dnn_Input))./2 + min(Dnn_Input);
        Hdnn_pre = pre_Received_data_DNN(1:32) + pre_Received_data_DNN(33:end)*1i;
        rx_dnn_ofdm_data(i,:) = rx_ofdm_data(i,:) - Hdnn_pre;
    end

    [PSDU_DL_decode_out] = rx_test2(rx_dnn_ofdm_data,symbol_num,Pilot_method,VCNum_method,Np,loc_pilot,phase,N_FFT,ValidCarrierNum,PSDU_BPC,wlanSymMap,PadBits_Num,PSDU_PB);
...
% 采集完数据 开始DNN训练
% [Train_out, Train_Info] = Train_DNN_V1(Train_data_dnn, Train_label_dnn, Valid_data_dnn, Valid_label_dnn);
